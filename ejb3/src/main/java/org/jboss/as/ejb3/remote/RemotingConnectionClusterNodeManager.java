...
class RemotingConnectionClusterNodeManager implements ClusterNodeManager {
    ...
    public EJBReceiver getEJBReceiver() {
        ...
        try {
            ...
            if (ejbClientConfiguration != null) {
                final URI connectionURI = new URI("remote://" + this.destinationHost + ":" + this.destinationPort);
                ...
                if (clusterConfiguration == null) {
                    ...
                    reconnectHandler = new ClusterNodeReconnectHandler(connectionURI, connectionCreationOptions, callbackHandler, channelCreationOptions, 5000);
                    ...
                    // create a re-connect handler (which will be used on connection breaking down)
                    reconnectHandler = new ClusterNodeReconnectHandler(connectionURI, connectionCreationOptions, callbackHandler, channelCreationOptions, timeout);
                }
    ...
    private class ClusterNodeReconnectHandler implements ReconnectHandler {
        ...
        ClusterNodeReconnectHandler(final URI connectionURI, final OptionMap connectionCreationOptions, final CallbackHandler callbackHandler, final OptionMap channelCreationOptions, final long connectionTimeoutInMillis) {
        ...
        public void reconnect() throws IOException {
            ...
            try {
                final IoFuture<Connection> futureConnection = endpoint.connect(uri, connectionCreationOptions, callbackHandler);
                ...
                logger.debug("Failed to re-connect to " + this.uri, e);
            }
            if (connection == null) {
                return;
            }
